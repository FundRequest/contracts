...
contract('FundRequestContract', function (accounts) {
  ...
  it('isValid should return true for whitelisted token for specific organization', async function () {
    ...
    let platformId = '1';
    await tokenWhitelistPrecondition.allow(platform, platformId, token, true);
    let isValid = await tokenWhitelistPrecondition.isValid(platform, platformId, token, 1, funder);
    expect(isValid).to.be.true;
  });

  it('isValid should return false for whitelisted token for specific organization when not allowed', async function () {
    let platform = web3.fromAscii('GITHUB');
    let platformId = '1';
    await tokenWhitelistPrecondition.allow(platform, platformId, token, false);
    let isValid = await tokenWhitelistPrecondition.isValid(platform, platformId, token, 1, funder);
    expect(isValid).to.be.false;
  });
