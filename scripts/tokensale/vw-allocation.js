...
const doThings = function (address, amount, _callback) {
	...
	if (amount === 0) {
		console.log('not doing ' + address + ' because there arent any vested tokens');
		_callback();
	} else {
...
let iterate = function (lines) {
	...
	if (allocation.length > 5) {
		let address = vw_address;
		let tokensVested = allocation[5];

		console.log('allocating tokens for ' + address + ' -> ' + tokensVested * Math.pow(10, 18));


		doThings(address, tokensVested * Math.pow(10, 18), function (_callback) {
...
fs.readFile('csv/presale2.csv', 'utf8', function (err, data) {
	if (err) throw err;
	iterate(data.split('\n'));
});